...
{
    public static string DateTimeFormat { get; set; } = Time.Formats.RFC3339Long;
    ...
    public static ParquetSchema BuildSchemaFromDataReader(IDataReader reader)
    {
        if (reader.IsClosed)
        {
    ...
    public static DataField GetParquetType(IDataReader reader, int index)
    {
        ...
        if (_dbToParquetTypeMap.ContainsKey(databaseType))
        {
            ...
            { dataField = new DataField(fieldName, parquetType, isNullable: true); }
            else
        ...
        if (_skipTypes.Contains(fieldClrType)) // Add blank string columns for data that refuses to map in testing.
        {
            try
            {
                ...
                return new DataField(fieldName, DataType.String, isNullable: true);
            }
            catch
            { }
        }
    ...
    public static object GetValueForParquet(
        ...
        if (_dbToParquetTypeMap.ContainsKey(normalizedDatabaseType))
        {
            switch (_dbToParquetTypeMap[normalizedDatabaseType])
            {
                ...
                case DataType.String:
                    ...
                    { return reader.GetDateTime(index).ToString(DateTimeFormat); }
...
#pragma warning restore CS0618 // Type or member is obsolete
    ...
    private static object GetComplexValueForParquet(this IDataReader reader, int index)
    {
        ...
        switch (value)
        {
            ...
            case DateTime dt: return dt.ToString(DateTimeFormat);
            case Guid g: return g.ToString();
            case MySqlDateTime mydt: return mydt.GetDateTime().ToString(DateTimeFormat);
    ...
    {
        try
        { return System.Text.Json.JsonSerializer.Serialize(value); } // fast & lower memory - handles 99% use cases
        catch
        {
            try
    ...
    public static long GoodEnoughThresholdLength { get; set; } = 24 * 1024; // 24 KB
    public static long MaxFileSize { get; set; } = 200 * 1000 * 1024; // 200 MB
    public static long MaxRowGroupSize { get; set; } = 1000;
    ...
    public static async Task<(int, int)> WriteEncryptedParquetFiles(
        IDataReader reader,
        ...
        CancellationToken token = default)
    {
        ...
        if (Directory.Exists(outputDirectory))
        {
            ...
            while (!reader.IsClosed)
            {
                ...
                rowCount += await WriteEncryptedFileAsync(
                    reader,
                    encryptionProvider,
                    schema,
                    normalizedDatabaseTypes,
    ...
    public static async Task<int> WriteEncryptedFileAsync(
        IDataReader reader,
        IStreamEncryptionProvider encryptionProvider,
        ParquetSchema schema,
        string[] normalizedDatabaseTypes,
        ...
        CancellationToken token = default)
    {
        ...
        while (!reader.IsClosed)
        {
            ...
            //
            // If we proceed past this point, postCompressedRowGroupSize
            // will be applied and write essentially one more full RowGroup to file.
            // If the RowGroup is sufficiently large enough, this could
            // drastically alter the expected file size.
            if (remainingAllowedFileSize < GoodEnoughThresholdLength)
            { break; }
        ...
        long? maxRowGroupSize = null)
    {
        ...
        while (!reader.IsClosed && reader.Read())
        {
            ...
            if (currentRowCount >= (maxRowGroupSize ?? MaxRowGroupSize)) return;
        }
