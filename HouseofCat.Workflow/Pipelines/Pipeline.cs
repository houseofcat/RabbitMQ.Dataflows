...
namespace HouseofCat.Library.Workflows.Pipelines
{
    ...
    public class Pipeline<TIn, TOut> : IPipeline<TIn, TOut>
    {
        ...
        {
            if (Ready) throw new InvalidOperationException(Constants.InvalidAddError);
            ...
            else
            {
                ...
                if (lastStep.IsAsync)
                {
                    ...
                    else { throw new InvalidOperationException(Constants.InvalidStepFound); }
                }
                else
                {
        ...
        {
            if (Ready) throw new InvalidOperationException(Constants.AlreadyFinalized);
            if (Steps.Count == 0) throw new InvalidOperationException(Constants.CantFinalize);
        ...
        {
            if (!Ready) throw new InvalidOperationException(Constants.NotFinalized);

            if (Steps[0].Block is ITargetBlock<TIn> firstStep)
            {
                _logger.LogTrace(Constants.Queued, _pipelineName);
        ...
        public async Task<bool> AwaitCompletionAsync()
        {
            ...
            if (Steps[0].Block is ITargetBlock<TIn> firstStep)
            {
                ...
                {
                    _logger.LogTrace(Constants.AwaitsCompletion, _pipelineName);
        ...
        private async Task SimplePipelineHealthTaskAsync()
        {
            ...
            while (true)
            {
                ...
                if (ex != null)
                { _logger.LogCritical(ex, Constants.Faulted, _pipelineName); }
                else  // No Steps are Faulted... Hooray!
                { _logger.LogDebug(Constants.Healthy, _pipelineName); }
            }
        }
    }
}
